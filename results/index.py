...
class TestPage(object):
    ...
    def __init__(self, db, config):
        ...
        if self.branch != 'develop':
            self.lab_only = False
        self.date, self.last_build_date, self.version, self.last_build_version \
                  = self.get_date_and_version()
...
""" % self.get_build_id()
    ...
    def get_topdir(self, branch, lab_only=False):
        k = 'LAB_ONLY_TOPDIR' if lab_only else 'TOPDIR'
        return os.path.join(self.config[k], branch)

    def get_last_build_date(self):
        """Get date of most recent nightly build"""
        s = os.readlink(os.path.join(self.get_topdir(self.branch), 'lastbuild'))
    ...
    def display_comp_plat_tests(self):
        def loglinks(plat, comp, lab_only):
            ...
            if comp.endswith(' examples'):
                ...
                comp = comp[:-11]
            link = "<li>%s</li>" % self.get_raw_log_link('%s/%s.%s.log' \
                                         % (plat, comp, build_type),
                                            lab_only, "The raw log file",
                                            remove_prefix=False)
            if build_type == 'test':
                link += "\n<li>%s</li>" \
                        % self.get_raw_log_link('%s/%s.build.log' \
                                            % (plat, comp),
                                            lab_only, "The build log file",
                                            remove_prefix=False)
            return link
        component_name, lab_only = self.get_component_from_id(self.db,
                                                              self.component)
        if not component_name:
            print "<p><b>Unknown component.</b></p>"
            return
        platform_name = self.get_platform_name_from_id(self.db, self.platform)
        ...
        print '<p>See also:</p>'
        print '<ul><li><a href="%s">Test results for this component on <b>all</b> platforms</a></li>' % self.get_link(page='comp')
    ...
    def display_component(self, component_id):
        ...
        db = BuildDatabase(self.db, self.config, self.date, self.lab_only,
                           self.branch)
        tests = db.get_all_component_tests(component_id)
        # Set default component for all test links
        self.component = component_id
        self.bench = None
        return render_template('component.html', component=component_name,
            build_id=self.get_build_id(),
            test_table="\n".join(
                self.display_tests(tests, include_component=False)))
    ...
    def display_all_failures(self):
        print "<h1>All test failures for build on %s</h1>" % self.get_build_id()
        db = BuildDatabase(self.db, self.config, self.date, self.lab_only,
                           self.branch)
        self.display_tests(db.get_all_failed_tests())

    def display_new_failures(self):
        print "<h1>New test failures for build on %s</h1>" % self.get_build_id()
        db = BuildDatabase(self.db, self.config, self.date, self.lab_only,
                           self.branch)
  ...
  var plot%d = plot_bench('bench_%d', [[%s], [%s]]);
    ...
    def display_platform(self, platform_id):
        plat_name = self.get_platform_name_from_id(self.db, platform_id)
        ...
        if self.lab_only:
            log_links = [
                self.get_raw_log_link('%s/' % plat_name, False,
                    ...
                    remove_prefix=False),
                self.get_raw_log_link('%s/' % plat_name, True,
            ...
            log_links = [
                self.get_raw_log_link('%s/' % plat_name, False,
                                      ...
                                      remove_prefix=False)]
        return render_template('platform.html', platform=p, log_links=log_links)
    ...
    def get_link(self, page=None, test=None, platform=None, date=None,
        ...
        elif page == 'benchfile' and bench is not None and platform is not None:
    ...
    def format_build_summary(self, summary, unit, arch, arch_id, unit_id):
        def make_cmake_loglink(cls, title, build_type, data, numfails=0,
            ...
            if build_type == 'test' \
               or (build_type == 'benchmark' and unit.endswith(' benchmarks')) \
        ...
        if s is None or s['state'] in ('SKIP', 'CMAKE_SKIP'):
            ...
            if unit.endswith(' examples'):
                return make_cmake_loglink(cls='moduleok',
                                  title="Examples ran successfully",
                                  build_type='example', data=s)
            elif unit.endswith(' benchmarks'):
                return make_cmake_loglink(cls='moduleok',
                                  title="Benchmarks ran successfully",
                                  build_type='benchmark', data=s)
            else:
                return make_cmake_loglink(cls='moduleok',
                                  title="Component built successfully",
                                  build_type='build', data=s)
        elif s['state'] == 'CMAKE_BUILD':
            return make_cmake_loglink(cls='modulebuild',
                                  title="Component failed to build",
                                  build_type='build', data=s)
            ...
            return make_cmake_loglink(cls='modulebuild',
                                  title="Component build did not complete",
                                  build_type='build', data=s)
            ...
            return make_cmake_loglink(cls='moduletest',
                                  title="Component failed %d %s" \
                                        % handle_plural(s['numfails'],
                                                        "test case") \
                                        + print_newfail(s),
            ...
            return make_cmake_loglink(cls='modulebuild',
                                  title="Component test did not complete",
                                  build_type='test', data=s)
        elif s['state'] == 'CMAKE_EXAMPLE':
            return make_cmake_loglink(cls='moduleexample',
                                  title="%d %s failed" \
                                        % handle_plural(s['numfails'],
                                                        "component example") \
                                        + print_newfail(s),
            ...
            return make_cmake_loglink(cls='modulebuild',
                                  title="Component examples did not complete",
                                  build_type='example', data=s)
        else:
    ...
    def toggle_failmap(self, show_failures, caption):
        if show_failures:
            return "<a title=\"Show only components or platforms that have " \
              "at least one failure\" onclick=\"toggle_visibility('failmap', " \
            ...
            if self.date == self.last_build_date:
                ...
                if self.branch == 'develop':
                    print 'Pre-built binaries ' \
                         '<a href="http://integrativemodeling.org/nightly/download/">' \
    ...
    def display_log(self):
        c = MySQLdb.cursors.DictCursor(self.db)
        arch_name = self.get_platform_name_from_id(self.db, self.platform)
        if not arch_name:
    ...
    def display_tests(self, cur, include_component=True, include_platform=True):
        ...
        if include_component:
            yield "<th>Component</th>"
        if include_platform:
            yield "<th>Platform</th>"
        yield '<th class="sorttable_nosort"><a title="Show/hide all output" ' \
              'onclick="toggle_all_detail(); return false;" ' \
              'id="dettog" class="dettog" href="#">[+]</a></th>'
        yield '<th title="Name of the Python or C++ file containing test ' \
              'cases">Name</th>'
        yield '<th title="Time (in seconds) that all tests in this file ' \
              'took to run">Runtime (s)</th>'
        yield '<th title="OK: all tests ran successfully; FAIL: at least ' \
        ...
        yield '<th title="Difference between this test and the same test ' \
        ...
        for n, row in enumerate(cur):
            ...
            if include_component:
                yield "<td>%s</td>" \
                    % self.get_component_link(row['unit_name'], row['unit_id'])
            if include_platform:
                yield get_platform_td(row['arch_name'])
            detail = row['detail']
            if detail is None or detail == '':
                detail = ''
                yield "<td></td>"
            else:
                yield '<td><a title="Show/hide output" ' \
            ...
            yield "<td><a href=\"%s\">%s</a>%s</td>" \
                  % (testlink, test_name, detail)
            yield "<td>%.2f</td> %s %s</tr>" \
...
%s
    ...
    def display_test(self):
        ...
        if row['state'] in OK_STATES:
            print "<tr><td>Previously failed on</td> <td>%s</td></tr>" \
                  % self.get_previous_test_link(self.db, self.test,
                                                self.platform,
                                                False)
        else:
    ...
    def display_branch_link(self):
        branch_links = [self.get_link(branch=x) for x in self.all_branches]
        print '<script type="text/javascript">'
